...
fn build_schema(required: Vec<String>, properties: Option<HashMap<String, Value>>) -> Option<Value> {
...
pub fn set_config_value_schema() -> ToolInputSchema {
    ...
    properties.insert("key".to_string(), create_prop("string", "Configuration key to set. Valid keys: allowedDirectories, blockedCommands, defaultShell, fileReadLineLimit, fileWriteLineLimit."));
    ...
    let required = vec!["key".to_string(), "value".to_string()];
    ToolInputSchema::new(required.clone(), build_schema(required, Some(properties)))
}
...
pub fn read_file_schema() -> ToolInputSchema {
    ...
    properties.insert("length".to_string(), json!({ // Optional, no default here
        "type": "integer",
...
pub fn read_multiple_files_schema() -> ToolInputSchema {
    ...
    let required = vec!["paths".to_string()];
    ToolInputSchema::new(required.clone(), build_schema(required, Some(properties)))
}
...
pub fn search_files_schema() -> ToolInputSchema {
    ...
    properties.insert("timeoutMs".to_string(), json!({ // Optional
        "type": "integer",
        "description": "Timeout in milliseconds. Default 30000 (30s)."
    }));
    let required = vec!["path".to_string(), "pattern".to_string()];
    ToolInputSchema::new(required.clone(), build_schema(required, Some(properties)))
}
...
pub fn search_code_schema() -> ToolInputSchema {
    ...
    properties.insert("file_pattern".to_string(), json!({ "type": "string", "description": "Glob pattern to filter files (e.g., \"*.rs\", \"!**/target/*\"). See ripgrep --glob."}));
    properties.insert("max_depth".to_string(), json!({ "type": "integer", "description": "Maximum search depth relative to the search path."}));
    ...
    properties.insert("timeoutMs".to_string(), json!({ "type": "integer", "description": "Timeout in milliseconds. Default 30000 (30s)."}));
    let required = vec!["pattern".to_string()];
    ToolInputSchema::new(required.clone(), build_schema(required, Some(properties)))
}
...
pub fn edit_block_schema() -> ToolInputSchema {
    ...
    let required = vec!["file_path".to_string(), "old_string".to_string(), "new_string".to_string()];
    ToolInputSchema::new(required.clone(), build_schema(required, Some(properties)))
}
...
pub fn execute_command_schema() -> ToolInputSchema {
    ...
    properties.insert("timeout_ms".to_string(), create_prop_with_default_int("integer", "Timeout in milliseconds for initial output. Command continues in background if exceeded. Default 1000ms.", 1000));
    properties.insert("shell".to_string(), json!({ "type": "string", "description": "Specific shell to use (e.g., 'bash', 'powershell'). Uses server's default shell if not set."}));
...
pub fn read_output_schema() -> ToolInputSchema {
    ...
    let required = vec!["session_id".to_string()];
    ToolInputSchema::new(required.clone(), build_schema(required, Some(properties)))
}
...
pub fn kill_process_schema() -> ToolInputSchema {
    let mut properties = HashMap::new();
    properties.insert("pid".to_string(), create_prop("integer", "Process ID (PID) to terminate.")); // Changed to integer
    let required = vec!["pid".to_string()];
    ToolInputSchema::new(required.clone(), build_schema(required, Some(properties)))
}
